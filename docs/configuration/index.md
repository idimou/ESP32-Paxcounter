...
See [`generic.h`](https://github.com/cyberman54/ESP32-Paxcounter/blob/master/config/hal/generic.h) for all options and for proper configuration of BME280/BME680.

=== "BME/ BMP Configuration"
	```c linenums="37" title="config/hal/generic.h"
	--8<-- "config/hal/generic.h:37:49"
	```
=== "SDS011 Configuration"
	```c linenums="51" title="config/hal/generic.h"
	--8<-- "config/hal/generic.h:51:56"
	```
=== "Custom Sensors Configuration"
	```c linenums="57" title="config/hal/generic.h"
	--8<-- "config/hal/generic.h:57:60"
	```

=== "Complete `generic.h`"
	```c linenums="1" title="config/hal/generic.h"
	--8<-- "config/hal/generic.h"
	```
...
```c linenums="102" title="config/paxcounter_orig.conf"
--8<-- "config/paxcounter_orig.conf:102:102"
```
...
```c linenums="88" title="config/paxcounter_orig.conf"
--8<-- "config/paxcounter_orig.conf:88:88"
```

Supported external time sources are GPS, LORAWAN network time and LORAWAN application timeserver time. Supported on board time sources are the RTC of ESP32 and a DS3231 RTC chip, both are kept sycned as fallback time sources. Time accuracy depends on board's time base which generates the pulse per second. Supported are GPS PPS, SQW output of RTC, and internal ESP32 hardware timer. Time base is selected by #defines in the board's hal file, see example in [`generic.h`](https://github.com/cyberman54/ESP32-Paxcounter/blob/master/config/hal/generic.h).

```c linenums="87" title="config/hal/generic.h"
--8<-- "config/hal/generic.h:87:96"
```
...
```c linenums="60" title="config/paxcounter_orig.conf"
--8<-- "config/paxcounter_orig.conf:60:60"
```
...
Data can be stored on a FAT32 (pre-)formatted SD-card if the board provides an SD card interface, either with SPI or MMC mode. To enable this feature, specify interface mode and hardware pins in board's hal file (`config/hal/<board.h\>`):
...
This is an example of a board with MMC SD-card interface: [https://www.aliexpress.com/item/32915894264.html](https://www.aliexpress.com/item/32915894264.html). For this board use file [`config/hal/ttgov21new.h`](https://github.com/cyberman54/ESP32-Paxcounter/blob/master/config/hal/ttgov21new.h) and add the lines given above.
...
In this case you choose the correct file for your ESP32-board in the config/hal-directory and add the lines given above. Edit the pin numbers given in the example, according to your wiring.
...
```c linenums="16" title="config/paxcounter_orig.conf"
--8<-- "config/paxcounter_orig.conf:16:16"
```
